...
CmdCallbackType cmdCallback,const char * entity_category, int entNumber,bool grStTopic,float max, float min){
  ...
  /* Ent. name is copied into internal buffer to be possible use in application local parameeter for ent. name*/
  char * alocEntName = new char[strlen(ent_name)+1];
  assert(alocEntName!=nullptr);
  strcpy(alocEntName,ent_name);
  m_enitiyDB[m_nrOFRegEnt]->ent_name=alocEntName;
